...
fn main() {
    ...
    // Build the Encapfn C MPK runtime:
    ...
    println!("cargo:rerun-if-changed=src/encapfn_mpk_rt.c");
    println!("cargo:rerun-if-changed=src/encapfn_mpk_rt.h");
    check_output_res(
        std::process::Command::new(&cc)
            .args([
                ...
                OsStr::new("src/encapfn_mpk_rt.c"),
                OsStr::new("-o"),
                out_path.join("libencapfn_mpk_rt.so").as_os_str(),
            ])
            .output(),
    ...
    println!("cargo:rerun-if-changed=src/encapfn_mpk_loader.c");
    println!("cargo:rerun-if-changed=src/encapfn_mpk_loader.h");
    check_output_res(
        std::process::Command::new(&cc)
            .args([
                OsStr::new("-g"),        // Produce debug symbols in the target's native format
                ...
                OsStr::new("-shared"),   // Produce a shared object
                OsStr::new("-static"),   // Prevent linking with shared libraries
                OsStr::new("src/encapfn_mpk_loader_stub.c"),
                OsStr::new("-o"),
                out_path.join("libencapfn_mpk_loader_stub.so").as_os_str(),
            ])
            .output(),
        "Failed to compile the Encapfn MPK loader into a shared library!",
    );
}
